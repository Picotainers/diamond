...
# Compatibility-first template for diamond.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/diamond" ]; then BIN="/opt/conda/bin/diamond"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo diamond | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'diamond*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/diamond
RUN chmod +x /usr/local/bin/diamond && rm -f /tmp/tool-entry-path
WORKDIR /data
